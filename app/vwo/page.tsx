...
export default function VwoPage() {
  const vwoLessen = lessen.filter((l) => l.niveau === "vwo" || l.niveau === "beide");
    ...
    <main className="min-h-screen">
      <header className="bg-white border-b border-gray-200 px-6 py-4 flex items-center justify-between">
        ...
        <nav className="hidden md:flex gap-6 text-sm text-gray-600">
          ...
          <Link href="/vwo" className="text-[#6c3fc5] font-semibold">VWO</Link>
        </nav>
      </header>

      <section className="bg-gradient-to-br from-[#6c3fc5] to-[#4f46e5] text-white px-6 py-12 text-center">
        <div className="text-sm font-medium text-purple-200 mb-2">🎓 VWO 4, 5 en 6</div>
        <h1 className="text-3xl md:text-4xl font-bold mb-3">Eindexamen Nederlands VWO</h1>
        <p className="text-purple-100 max-w-lg mx-auto">
          Verdieping in alle examenonderdelen. Meer nadruk op analyse, abstractie en 4F-niveau.
        </p>
      </section>

      <section className="max-w-4xl mx-auto px-6 py-10">
        <div className="bg-[#f3eeff] rounded-xl p-4 mb-8 text-sm text-[#6c3fc5]">
          <strong>Het eindexamen VWO Nederlands</strong> bestaat uit een Centraal Examen (CE) met leesvaardigheid
          en een Schoolexamen (SE) met schrijfvaardigheid, literatuur en mondeling. VWO vraagt meer diepgang en abstractievermogen dan HAVO.
        </div>

        {vaardigheden.map((vaardigheid) => {
          const modulesVoorVaardigheid = vwoLessen.filter((l) => l.vaardigheid === vaardigheid.naam);
